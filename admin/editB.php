...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["short_description"])) {
            ...
            $params[] = $_POST["short_description"];
        }
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
            ...
            // Add the file update to the statements
            $updateFields[] = "file=?";
            $params[] = $image;
        } else {
            // Keep the old file if no new file is uploaded
            $updateFields[] = "file=?";
            $params[] = $_POST['existing_image'];
        }

        // Construct the SQL statement dynamically
        $sql = "UPDATE bundle SET " . implode(", ", $updateFields) . " WHERE id=?";
        $params[] = $id;
        ...
        $types = str_repeat('s', count($params) - 1) . 'i'; // all are strings except the last one (id)
        $stmt->bind_param($types, ...$params);

        // Execute the statement
        if ($stmt->execute()) {
            header("Location: edit_update_bundleB.php");
        } else {
    ...
    $result = $conn->query("SELECT * FROM bundle WHERE id = $id");
...
<body>
    <div class="container">
        ...
        <a class="back-link" href="edit_update_bundleB.php">Back to Bundle List</a>
    </div>
</body>
</html>
