...
<main id="main" class="main">
 ...
 $database = "adminrogers_connect";
    ...
    if (isset($_GET['delete'])) {
        ...
        $deleteStmt = $conn->prepare("DELETE FROM bottom_banners WHERE id = ?");
        $deleteStmt->bind_param("i", $id);
        $deleteStmt->execute();
        $deleteStmt->close();
        echo "<script>
        alert('Record deleted successfully!');
        window.location.href = 'edit_bottom_banners.php';
      </script>";
        exit();
    }

    // Fetch records from the database
    $result = $conn->query("SELECT * FROM bottom_banners");
    ?>
              ...
              <table>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td><a href='uploads/" . htmlspecialchars($row['file']) . "' target='_blank'><img src='uploads/" . htmlspecialchars($row['file']) . "' alt='Image' style='width:100px;height:100px;'></a></td>";
                        echo "<td><a href='editbottom_banners.php?id=" . $row['id'] . "' class='edit-btn'>Edit</a></td>";
