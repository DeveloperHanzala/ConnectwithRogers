...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $deleteStmt = $conn->prepare("DELETE FROM limited_offers WHERE id = ?");
    $deleteStmt->bind_param("i", $id);
    $deleteStmt->execute();
    $deleteStmt->close();
    echo "<script>
        alert('Record deleted successfully!');
        window.location.href = 'edit_limited_offers.php';
      </script>";
    exit();
}

// Fetch records from the database
$result = $conn->query("SELECT * FROM limited_offers");
    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body">
              <h1 class="card-title" style="font-weight:bold;font-size:30px;">Connect With Rogers</h1>
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Edit button
                echo "<td><a href='limitededit.php?id=" . $row['id'] . "' class='edit-btn'>Edit</a></td>";
...
function redirectToPage(value) {
    if (value === "B2C") {
        window.location.href = "edit_limited_offersB.php"; // Redirect to index.php if B2C is selected
    } else if (value === "Connect with rogers") {
        window.location.href = "edit_limited_offers.php"; // Redirect to booking.php if Connect with rogers is selected
    }
}
</script>
