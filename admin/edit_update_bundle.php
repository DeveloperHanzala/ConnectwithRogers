...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $deleteStmt = $conn->prepare("DELETE FROM bundle WHERE id = ?");
    $deleteStmt->bind_param("i", $id);
    $deleteStmt->execute();
    $deleteStmt->close();
    echo "<script>
        alert('Record deleted successfully!');
        window.location.href = 'edit_update_bundle.php';
      </script>";
    exit();
}

// Fetch records from the database
$result = $conn->query("SELECT * FROM bundle");
...
function redirectToPage(value) {
    if (value === "B2C") {
        window.location.href = "edit_update_bundleB.php"; // Redirect to index.php if B2C is selected
    } else if (value === "Connect with rogers") {
        window.location.href = "edit_update_bundle.php"; // Redirect to booking.php if Connect with rogers is selected
    }
}
</script>
</body>
