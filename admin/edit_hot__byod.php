...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $deleteStmt = $conn->prepare("DELETE FROM hot_byod WHERE id = ?");
    $deleteStmt->bind_param("i", $id);
    $deleteStmt->execute();
    $deleteStmt->close();
    echo "<script>
        alert('Record deleted successfully!');
        window.location.href = 'edit_hot__byod.php';
      </script>";
    exit();
}

// Fetch records from the database
$result = $conn->query("SELECT * FROM hot_byod");
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                // Edit button
                echo "<td><a href='byodedit.php?id=" . $row['id'] . "' class='edit-btn'>Edit</a></td>";
...
function redirectToPage(value) {
    if (value === "B2C") {
        window.location.href = "edit_hot__byodB.php"; // Redirect to index.php if B2C is selected
    } else if (value === "Connect with rogers") {
        window.location.href = "edit_hot__byod.php"; // Redirect to booking.php if Connect with rogers is selected
    }
}
</script>
</body>
