...
if (isset($_GET['id'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($updates)) {
            $sql = "UPDATE bottom_banners SET " . implode(", ", $updates) . " WHERE id=?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $id); // Bind the ID parameter

            if ($stmt->execute()) {
                // Redirect back to the main page after the update
                header("Location: edit_bottom_banners.php");
                exit();
            } else {
    ...
    $result = $conn->query("SELECT * FROM bottom_banners WHERE id = $id");
...
<body>
    <div class="container">
        ...
        <a class="back-link" href="edit_top_banners.php">Back to Banners List</a>
    </div>
</body>
</html>
